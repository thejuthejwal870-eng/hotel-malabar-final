  ...
  return (
    <div className="customer-reference-app min-h-screen bg-[#07170e] text-[#fcfaf6] flex flex-col font-sans selection:bg-[#cba135] selection:text-[#0a1f13]">
      ...
      <section className="customer-reference-hero relative overflow-hidden border-b border-[#c9a338]/25">
        ...
        <div className="relative max-w-7xl mx-auto px-5 sm:px-8 py-10 sm:py-14 lg:py-16 min-h-[280px] sm:min-h-[350px] flex items-center">
          <div className="max-w-xl">
            <div className="inline-flex items-center gap-2 rounded-full border border-[#dfb64c]/50 bg-[#07170e]/70 px-3 py-1 text-[10px] uppercase tracking-[0.22em] text-[#dfb64c] backdrop-blur-sm"><UtensilsCrossed className="w-3.5 h-3.5" /> Authentic Malabar Cuisine</div>
            <h1 className="font-brand text-3xl sm:text-5xl lg:text-6xl leading-[.95] mt-3"><span className="block text-[#fcfaf6]">Authentic Malabar</span><span className="block text-[#dfb64c]">Taste Delivered</span></h1>
            <p className="mt-3 text-xs sm:text-sm text-[#d5e1d9] max-w-lg leading-6">Freshly cooked Kerala favourites, biryani, tandoori, shawarma and more — delivered with care.</p>
            <button onClick={() => document.getElementById('most-ordered-section')?.scrollIntoView({ behavior: 'smooth' })} className="mt-5 inline-flex items-center gap-2 rounded-lg bg-[#dfb64c] text-[#07170e] font-bold text-xs px-4 py-2.5 shadow-lg hover:bg-[#ebca68]">Order Now <ArrowRight className="w-4 h-4" /></button>
          </div>
        </div>
      </section>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 py-5 sm:py-6">
        ...
        {activeOrdersCount > 0 && customerOrders[0] && <div onClick={() => setActiveTrackingOrderId(customerOrders[0].id)} className="mb-5 p-3.5 bg-gradient-to-r from-[#143d26] via-[#1a4f32] to-[#143d26] border-2 border-[#dfb64c] rounded-2xl shadow-xl flex items-center justify-between gap-3 cursor-pointer"><div className="flex items-center gap-3"><div className="w-9 h-9 rounded-full bg-[#dfb64c] text-[#0a1f13] flex items-center justify-center font-bold"><Clock className="w-4 h-4" /></div><div><span className="text-[10px] font-mono font-bold text-[#dfb64c]">ACTIVE ORDER {customerOrders[0].orderNumber}</span><h4 className="text-xs font-semibold text-[#fcfaf6]">Status: {customerOrders[0].status}</h4></div></div><span className="bg-[#0a1f13] text-[#dfb64c] text-[10px] font-semibold px-2.5 py-1.5 rounded-lg border border-[#cba135]/50">Track Live →</span></div>}
        {filteredMostOrdered.length > 0 && <section id="most-ordered-section" className="mb-8 pb-6 border-b border-[#1b432a]"><div className="flex flex-wrap items-center justify-between gap-2 mb-4"><div><h2 className="font-brand text-xl sm:text-2xl font-bold text-[#fcfaf6] flex items-center gap-2"><span>🔥</span><span>Most Ordered</span></h2><p className="text-[11px] text-[#8ea896] mt-0.5">Customer favourites from our authentic kitchen.</p></div><span className="text-[10px] text-[#dfb64c] bg-[#143d26] border border-[#235836] px-2.5 py-1 rounded-full font-bold">Customer Favorites</span></div><div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{filteredMostOrdered.map(item => { const cat = categories.find(c => c.id === item.categoryId); const cartItem = cartItems.find(ci => ci.menuItem.id === item.id); return <FoodCard key={`most-ordered-${item.id}`} item={item} categoryName={cat?.name} cartQuantity={cartItem?.quantity || 0} isRestaurantOpen={deliverySettings.isRestaurantOpen} isCategoryOpen={cat?.isActive !== false} onAddToCart={handleAddToCart} onUpdateQuantity={handleUpdateCartQuantity} />; })}</div></section>}
        <div className="flex flex-wrap items-center justify-between gap-2 mb-4"><div><h2 className="font-brand text-xl sm:text-2xl font-bold text-[#fcfaf6]">{selectedCategoryId === 'all' ? 'Authentic Malabar Menu' : categories.find(c => c.id === selectedCategoryId)?.name || 'Food Menu'}</h2><p className="text-[11px] text-[#8ea896] mt-0.5">Freshly prepared Kerala and Malabar dishes.</p></div><span className="text-[10px] text-[#c9dcce] bg-[#113320] border border-[#214f34] px-2.5 py-1 rounded-full">{filteredMenuItems.length} dishes</span></div>
        {filteredMenuItems.length === 0 ? <div className="bg-[#0f2d1c] border border-[#235836] rounded-2xl p-10 text-center text-[#8ea896] my-8"><p className="text-sm font-semibold text-[#fcfaf6] mb-1">No food items found</p><p className="text-xs">Try a different dish or category.</p></div> : <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">{filteredMenuItems.map(item => { const cat = categories.find(c => c.id === item.categoryId); const cartItem = cartItems.find(ci => ci.menuItem.id === item.id); return <FoodCard key={item.id} item={item} categoryName={cat?.name} cartQuantity={cartItem?.quantity || 0} isRestaurantOpen={deliverySettings.isRestaurantOpen} isCategoryOpen={cat?.isActive !== false} onAddToCart={handleAddToCart} onUpdateQuantity={handleUpdateCartQuantity} />; })}</div>}
      </main>
      {cartItems.length > 0 && !isCartOpen && <div className="fixed bottom-4 inset-x-4 max-w-lg mx-auto z-30"><button onClick={() => setIsCartOpen(true)} className="w-full bg-gradient-to-r from-[#dfb64c] to-[#cba135] text-[#0a1f13] font-bold py-3 px-5 rounded-2xl shadow-2xl flex items-center justify-between gap-2"><div className="flex items-center gap-2"><span className="bg-[#0a1f13] text-[#dfb64c] text-xs font-mono font-bold px-2.5 py-1 rounded-lg">{cartTotalItemsCount} {cartTotalItemsCount === 1 ? 'item' : 'items'}</span><span className="text-sm font-semibold">View Order Cart</span></div><div className="flex items-center gap-1.5 font-mono text-base"><span>₹{cartFoodTotal}</span><span>→</span></div></button></div>}
