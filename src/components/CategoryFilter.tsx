...
export const CategoryFilter: React.FC<CategoryFilterProps> = memo(({ categories, selectedCategoryId, onSelectCategory, categoryCounts }) => (
  <div className="hm-category-strip w-full bg-[#fbf8f0] border-b border-[#d9cfbb] sticky top-[108px] sm:top-[68px] z-30 shadow-sm">
    <div className="max-w-6xl mx-auto px-3 sm:px-4 py-2.5">
      <div className="flex items-center gap-3 overflow-x-auto pb-0.5 no-scrollbar">
        <button onClick={() => onSelectCategory('all')} className={`hm-category-chip shrink-0 flex flex-col items-center gap-1 min-w-[58px] ${selectedCategoryId === 'all' ? 'is-active' : ''}`} aria-label="All menu">
        ...
        {categories.map((cat) => {
          ...
          const image = cat.imageUrl?.trim() || categoryImageByName[cat.name.trim().toLowerCase()];
          return (
            <button key={cat.id} onClick={() => onSelectCategory(cat.id)} disabled={isClosed} className={`hm-category-chip shrink-0 flex flex-col items-center gap-1 min-w-[64px] ${isSelected ? 'is-active' : ''} ${isClosed ? 'is-closed' : ''}`} aria-label={`${cat.name}${count ? `, ${count} dishes` : ''}`}>
